namespace WebAPI.Controllers
{
    ...
    public class UsersController : ApiController
    {
        ...
        {
            var res = this.users.All.Where(x => x.Expire == false).Select(x => new { x.UserData.FirstName, x.UserData.LastName, x.UserName, x.Id, x.UserData.isMale, CityName= x.UserData.Adress.City.Name,CountryName=x.UserData.Adress.City.Country.Name, x.UserData.Email }).OrderBy(x => x.LastName).ThenBy(x => x.FirstName).ToList();
            return this.Ok(res);
        ...
        public IHttpActionResult Get(int id)
        {
            ...
            return this.Ok(new { id, res.UserName, res.Email, res.FirstName, res.LastName, res.isMale, res.Adress, res.CityName, res.CountryName });
        }
